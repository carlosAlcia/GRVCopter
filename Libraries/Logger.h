...
/***** How to create a new Log Item:
    In Logger_Constants.h add a new ID after the last one.
    When the item should be logged, call save_float_data with the new ID and the value.
    If necessary, can create a new function specific to log this new item. save_float_data is a generic function for any log item.
    In Bin2Mat.py, add the new log item name to the list to be able to convert from .bin to .mat.
    The Logs Items are always a float value. Cannot log text or boolean. Look for an alternative to log this information if needed.
*****/
...
class Logger {
    ...
    private:
        //@brief Create a new binary log file
        void create_file(){
            using namespace std;

            //Create log file as .bin
        ...
        //@brief Function to get the bytes from a log item. 
