...
void Controller::run_stabilize_control(Force& forces, Torques& torques){
    ...
    Force force_xyz_n;
    force_xyz_n.z() = pow(common.get_rc()->get_throttle()+1,2)*0.25;
    //std::cout << "Throttle Sta: " << common.get_rc()->get_throttle() << std::endl;
    ...
    if (UAV::fully_actuated){
        ...
        //std::cout << "Fx: " << force_xyz_n.x() << "   Fy:" << force_xyz_n.y() << std::endl;
    ...
    forces = force_xyz_n;
    //std::cout << "Force Z Stabilize: " << forces.z() << std::endl;
...
void Controller::run_altitude_control(Force& forces, Torques& torques){
    ...
    if (UAV::fully_actuated){
        ...
        Force from_angles;
        from_angles = Attitude::get_forces_by_angles(*common.get_target_attitude());
        force_xyz_n.x() = from_angles[0]*UAV::mass_kg*GRAVITY*2;
        force_xyz_n.y() = from_angles[1]*UAV::mass_kg*GRAVITY*2;
    ...
    //Add feedforward from RC controller as Stabilize control. This way there is no initial gap when switching from stabilize to altitude.
    force_xyz_n.z() += hover_throttle;
    force_xyz_n.apply_limits(100.0, 100.0, 1.0);
...
void Controller::set_hover_throttle(){
    hover_throttle = pow(common.get_rc()->get_throttle()+1,2)*0.25;
}
