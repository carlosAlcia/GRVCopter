...
public:
    ...
    static void get_forces_each_motor(Mixer* mixer, Force* forces, Torques* torques, float* force_each_motor){
        //Get the mixer:
    ...
    static void force_to_pwm(float *forces_each_motor, float* pwms){
        for (int i = 0; i < UAV::num_motors; i++){
            int pwm = (int) (UAV::lower_pwm*(1.0+forces_each_motor[i]));
            saturation(pwm,1000,2000);
            pwms[i] = pwm;
        }
    }
