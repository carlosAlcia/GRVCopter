...
void MSG_GRVCOPTER::pack_param_info_message(int param_number, float value, const char* name_param, Message_Bytes* msg)
{
    memset(msg, 0, sizeof(Message_Bytes));
    msg->MSG_ID.value = PARAM_INFO_MSG_ID;
    msg->TIME.value = get_time_us();
    msg->DATA[0].value = (float)param_number;
    
    memccpy(msg->DATA[1].bytes, (name_param), '\0',52);

    
    msg->DATA[14].value = value;
}

void MSG_GRVCOPTER::pack_number_param_message(int number_params, Message_Bytes *msg)
{
    memset(msg, 0, sizeof(Message_Bytes));
    msg->MSG_ID.value = NUMBER_PARAMS_MSG_ID;
    msg->TIME.value = get_time_us();
    msg->DATA[0].value = (float)number_params;
}

void MSG_GRVCOPTER::pack_generic_message(int _id, float* values, Message_Bytes* msg){
    memset(msg, 0, sizeof(Message_Bytes));
    msg->MSG_ID.value = _id;
    msg->TIME.value = get_time_us();
    for(int i = 0; i < MSG_DATA_LENGTH; i++){
        msg->DATA[i].value = (float)values[i];
    }
}
